# WentUrc_ASCII_Art_Tool/cli.py

import os
import click
from WentUrc_ASCII_Art_Tool.image_processing import converter_txt, converter_png
from WentUrc_ASCII_Art_Tool.video_processing import converter_txt as video_converter_txt, converter_video
from WentUrc_ASCII_Art_Tool.playback import player
from WentUrc_ASCII_Art_Tool.utils.file_utils import is_valid_file
from WentUrc_ASCII_Art_Tool.config import config, logger

@click.command()
def main():
    """
    WentUrc ASCII Art Tool - 支持图片和视频处理，以及播放 .txt 文件
    """
    # 欢迎信息
    click.echo("=" * 50)
    click.echo("欢迎使用 WentUrc ASCII Art Tool！")
...
def handle_image_processing():
    ...
    if mode_choice == "1":
        ...
        if is_valid_file(image_path, [".jpg", ".jpeg", ".png"]):
            try:
                width_input = input(f"请输入 ASCII 图的宽度 (默认: {config.get_default_setting('ascii_width_txt')}): ").strip()
                new_width = int(width_input) if width_input.isdigit() else config.get_default_setting('ascii_width_txt')
                ascii_art = converter_txt.convert_image_to_ascii(image_path, new_width=new_width)
                if ascii_art:
                    converter_txt.save_ascii_to_file(ascii_art)
                    click.echo(f"ASCII 字符文件生成完成！结果已保存到 {config.get('output_directories').get('image_txt')}/")
        ...
        if is_valid_file(image_path, [".jpg", ".jpeg", ".png"]):
            try:
                target_width = input(f"请输入 ASCII 图的宽度 (默认: {config.get_default_setting('ascii_width_png')}): ").strip()
                target_width = int(target_width) if target_width.isdigit() else config.get_default_setting('ascii_width_png')
                converter_png.convert_and_save(image_path, target_width=target_width)
                click.echo(f"图片转换完成！结果已保存到 {config.get('output_directories').get('image_png')}/")
            except Exception as e:
                logger.exception("图片处理失败")
                click.echo(f"图片处理失败: {e}")
        else:
...
def handle_video_processing():
    ...
    if mode_choice == "1":
        ...
        if is_valid_file(video_path, [".mp4", ".avi", ".mkv"]):
            try:
                video_converter_txt.video_to_ascii(video_path, new_width=config.get_default_setting('ascii_width_txt'))
                video_name = os.path.splitext(os.path.basename(video_path))[0]
                video_output_dir = os.path.join(config.get('output_directories').get('video'), video_name)
        ...
        try:
            ...
            click.echo(f"ASCII 视频生成成功！结果已保存至 {config.get('output_directories').get('video')}/")
        except Exception as e:
            logger.exception("视频处理失败")
            click.echo(f"视频处理失败: {e}")
    else:
...
def handle_txt_playback():
    ...
    播放 .txt 文件夹内容。
    """
    folder_path = input("请输入存放 .txt 文件的文件夹路径 (示例: ./output/video/video_name): ").strip()
    if os.path.isdir(folder_path):
        try:
            fps_input = input(f"请输入播放帧率 (默认: {config.get_default_setting('video_fps')}): ").strip()
