...
| 文件 | 职责 |
| --- | --- |
| [`main.py`](../main.py) | CLI 配置查找、参数解析和顶层退出码处理 |
...
| [`campus_net/runner.py`](../campus_net/runner.py) | 串联新版流程并处理不确定 POST 的只读状态核对 |
| [`campus_net/legacy.py`](../campus_net/legacy.py) | 保留旧版 ePortal 登录流程 |
| [`campus_net/gui_core.py`](../campus_net/gui_core.py) | GUI 后台任务、事件队列、验证码桥接、接口枚举和表单校验 |
...
IPv4 接口枚举固定调用 `%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe`，避免当前目录中的同名程序参与 Windows 可执行文件搜索。PowerShell 枚举值在 JSON 中可能是 `0/1` 数字，也可能是状态字符串；解析层统一归一化，再在界面显示为“已连接”“未连接”或“正在认证”。枚举进程超时、失败或后台线程无法启动时都会恢复刷新按钮，用户仍可直接填写接口索引。
...
PyInstaller 保留两个独立入口：`build.spec` 生成控制台版 `Auto-Connect-CampusNet.exe`，`gui.spec` 生成窗口版 `Auto-Connect-CampusNet-GUI.exe`。两个 spec 的 `datas` 都为空，不会把本地 `config.json` 或备份嵌入可执行文件。
...
| `2` | 接口状态无法确认，或旧版模式不支持 `--probe-only` |
...
- `test_captive.py`：真实脚本形态、入口参数、origin/path、歧义响应、在线指纹和 Windows socket 绑定；
...
- `test_application.py`：CLI/GUI 共用协议分派和错误分类。

项目级验证命令：

```powershell
uv run python -m unittest discover -s tests -p "test_*.py" -v
uv run python main.py --probe-only
uv run pyinstaller --clean --noconfirm build.spec
uv run pyinstaller --clean --noconfirm gui.spec
```
